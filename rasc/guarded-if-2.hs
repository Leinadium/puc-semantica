...
evalExp (ExpGuarded exps) env k = aux exps
  where aux [] = "no match"
        aux ((c,e):xs) = evalExp c env k'
          where k' v = if isTrue(v) then evalExp e env k
                       else aux xs
                                                                          -- 103
                                                                          -- 104
----------------------------------------------------------------------    -- 105
...
f5 = ExpGuarded [((ExpK 0), ExpK(0)),((ExpAdd (ExpK 0) (ExpK 0)), (ExpK 2)),((ExpK 1), ExpK(1))]
                                                                          -- 127
                                                                          -- 128
                                                                          -- 129
-- code to show the final value of an expression                          -- 130
main :: IO ()                                                             -- 131
main = print (evalExp f5 emptyEnv k)                                    -- 132
